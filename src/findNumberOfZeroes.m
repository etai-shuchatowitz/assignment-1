...
function [numberOfZeroes zeroes] = findNumberOfZeroes(p2)
  tally = 1;                              % start counting at 1 (because Matlab starts indices at 1)
  prev = 0;
  current = 0;
  zeroes = [];
  for i = -pi:.01:pi                      % iterate the function over -pi to pi
    current = evaluateStruts(i, p2);      % evaluate the function at the given value 
    if prev*current < 0                   % if there has been a sign change, find the zero using bisection
      zeroes(tally) = bisection(@evaluateStruts, i-.01, i, .00001, p2);
      tally = tally+1;
     end
     prev = current;
  end
numberOfZeroes = tally-1;
end
